...
export default function TaskList() {
  const { isLoggedIn, tasks, userId, categories, subtasks } = useContext(
    DataStoreContext
  );
  ...
  useEffect(() => {
    ...
    for (taskValue of filteredTasks) {
      if (taskToCategoryMap.get(taskValue.category_id)) {
        taskToCategoryMap.set(
          taskValue.category_id,
          taskToCategoryMap.get(taskValue.category_id).concat([taskValue])
        );
      } else {
    ...
    for (subtaskValue of subtasks) {
      if (subtaskToTaskMap.get(subtaskValue.task_id)) {
        subtaskToTaskMap.set(
          subtaskValue.task_id,
          subtaskToTaskMap.get(subtaskValue.task_id).concat([subtaskValue])
        );
      }
    }
    ...
    for (const [index, value] of filteredTasks.entries()) {
      ...
      setUserTasks((userTasks) => [
        ...userTasks,
        showCategory === value.category_id && (
          <TaskItem
            key={value.id}
            id={value.id}
            index={index}
  ...
  return !tasksAvailable ? (
    ...
    <div>
      ...
      <div className="row">
        <div className="col col-md-4 taskContainer">{userCategories}</div>
        <div className="col col-md-6 taskItem">{userTasks}</div>
      </div>
    </div>
  );
}
